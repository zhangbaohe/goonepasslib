...
func NewVault(vaultPath string, security VaultSecurity) (Vault, error) {
	...
	err = jsonutilWriteFile(dataDir+"/contents.js", []string{})
	if err != nil {
...
func (vault *Vault) Unlock(pwd string) error {
	...
	err := jsonutilReadFile(vault.Path+"/encryptionKeys.js", &keyList)
	if err != nil {
...
func saveEncryptionKeys(dataDir string, keyList encryptionKeys) (err error) {
	err = jsonutilWriteFile(dataDir+"/encryptionKeys.js", keyList)
	if err != nil {
		return
	}
...
func (vault *Vault) SetMasterPassword(currentPwd string, newPwd string) error {
	...
	err := jsonutilReadFile(keyFilePath, &keyList)
	if err != nil {
		return errors.New("Failed to read encryption key file")
	}
...
func (item *Item) removeDataFiles() error {
	...
	err := jsonutilReadFile(contentsFilePath, &contentsEntries)
	if err != nil {
	...
	err = jsonutilWriteFile(contentsFilePath, newContentsEntries)
	if err != nil {
...
func (item *Item) Save() error {
	...
	itemPath := item.Path()
	err := jsonutilWriteFile(itemPath, item)
	if err != nil {
	...
	err = jsonutilWriteFile(contentsFilePath, contentsEntries)
	if err != nil {
		return fmt.Errorf("Failed to update contents.js: %v", err)
	}
...
func (vault *Vault) LoadItem(uuid string) (Item, error) {
	...
	err := jsonutilReadFile(vault.Path+"/"+uuid+".1password", &item)
	if err != nil {
		return Item{}, err
	}
	return item, nil
...
func (vault *Vault) ListItems() ([]Item, error) {
	...
	for _, item := range dirEntries {
		if path.Ext(item.Name()) == ".1password" {
			...
			err := jsonutilReadFile(vault.Path+"/"+item.Name(), &itemData)
			if err != nil {
...
func writePlistFile(path string, in interface{}) error {
	return jsonutilMarshalToFile(path, in, plistMarshal)
}
